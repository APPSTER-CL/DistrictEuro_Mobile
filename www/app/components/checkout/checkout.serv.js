angular.module('districteuro.checkout.services', [])
...
.service('CheckoutService', function ($http, $q){
  ...
  this.saveUserSelectedCard = function(card){
    window.localStorage.districteuro_selected_card = JSON.stringify(card);
  }
  this.saveUserSelectedAddress = function(address){
    window.localStorage.districteuro_selected_address = JSON.stringify(address);
  }
  this.getUserSelectedCard = function(){
    return JSON.parse(window.localStorage.districteuro_selected_card || '[]');
  };
  this.getUserSelectedAddress = function(){
    return JSON.parse(window.localStorage.districteuro_selected_address || '[]');
  };
})
